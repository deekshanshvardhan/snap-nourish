...
const slotDotColors: Record<string, string> = {
  ...
  lunch: "bg-accent",
  snack: "bg-nutrition-fat",
  dinner: "bg-nutrition-protein",
};
...
const MealTimeline = ({ meals, onLogMeal, onQuickLog, pinnedMeals = {}, onMealUpdated }: MealTimelineProps) => {
  ...
  return (
    <div className="relative pl-7">
      <div className="absolute left-[10px] top-4 bottom-4 w-[2px] bg-border rounded-full" />
      ...
      <div className="space-y-3">
        {slots.map((slot, i) => {
          ...
          return (
            ...
            >
              <div
                className={`absolute -left-7 top-[18px] w-5 h-5 rounded-full flex items-center justify-center z-10 border-2 border-background ${
                  hasMeals ? slotDotColors[slot.key] : "bg-muted"
                }`}
              >
                {hasMeals && <div className="w-1.5 h-1.5 rounded-full bg-white" />}
              </div>

              <div className="bg-card rounded-2xl border border-border overflow-hidden hover:shadow-sm transition-shadow">
                <div className="px-4 py-3 flex items-center justify-between">
                  ...
                  <button
                    ...
                    aria-label={`Log meal for ${slot.label}`}
                  >
                ...
                {hasMeals && (
                  <div className="px-4 pb-2 space-y-2">
                    {slot.meals.map((meal) => {
                      ...
                      const ago = timeAgo(meal.timestamp);
                ...
                {slot.meals.length === 0 && pinned.length === 0 && (
                  <div className="px-4 pb-3">
                    <button
                      onClick={() => onLogMeal(slot.key)}
                      className="w-full py-3 rounded-xl border border-dashed border-border text-xs text-muted-foreground font-body hover:bg-secondary/50 hover:border-primary/30 transition-all group"
                      aria-label={`Log a meal for ${slot.label}`}
                    >
                      <motion.span
                        animate={{ y: [0, -2, 0] }}
